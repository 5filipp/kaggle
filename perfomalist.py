...
def check_Data(
        data):  # checks recieved data for correct formating and returns either error string line or int amount of parameters
    lines = data.count("\n")
...
def do_math(data, param_number):  # does data calculation returns dictionary of output
    ...
    if param_number == 0:
        statistical_band = 95 / 100
        eV_threshold_percent = 5 / 100
        seasonality = 7
    if param_number == 1:
    ...
    for b in EV_normalized[1:]:
        if EV_normalized[z] * EV_normalized[z - 1] <= 0 and EV_normalized[z] != EV_normalized[z - 1] and EV_normalized[
            z - 1] == 0:
